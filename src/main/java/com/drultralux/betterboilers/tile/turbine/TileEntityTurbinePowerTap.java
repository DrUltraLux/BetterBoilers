...
public class TileEntityTurbinePowerTap extends TileEntityTurbinePart implements ITickable {
    ...
    public void update() {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            int energyTransfer = getController().energyStorage.extractEnergy(BBConfig.turbineOut, true);
            if (energyTransfer != 0) {
                int qty = cap.receiveEnergy(BBConfig.turbineOut, false);
                if (qty > 0) getController().energyStorage.extractEnergy(qty, false);
            }
        }
    }
    ...
    public <T> T getCapability(Capability<T> capability, @Nullable EnumFacing facing) {
        ...
        if (capability == CapabilityEnergy.ENERGY) {
            return (T) CapabilityEnergy.ENERGY.cast(getController().energyStorage);
        } else {
