...
public class TileEntityBoilerPump extends TileEntityBoilerPart implements ITickable {
    ...
    public void update() {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            FluidStack drain = getController().getTankSteam().drain(BBConfig.pumpDrain, false);
            if (drain != null) {
                int qty = cap.fill(drain, true);
                if (qty > 0) getController().getTankSteam().drain(qty, true);
            }
        }
    }
